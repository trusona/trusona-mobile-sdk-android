...
dependencies {
  // other dependencies
  compile "com.trusona.android:mobile-sdk:6.5.4"

  // the following is only required if you will be using the Trusona Passport SDK
  //compile "com.trusona.android:passport-sdk:6.5.4"
}
```
...
The latter will contain the device's status, the `device identifier` and  a Signed JWT containing the device identifier in the `sub` claim, if one was generated.
